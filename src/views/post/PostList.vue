<template>
  <page-view>
    <a-row :gutter="12">
      <a-col :xl="4" :lg="6" :md="12" :sm="24" :xs="24" class="mb-3">
        <a-card title="文章分类" :bodyStyle="{ padding: '16px' }">
          <a-spin :spinning="categoriesLoading">
            <category-tree :checkable="false" :categories="categories" @select="onSelectCategory" />
          </a-spin>
        </a-card>
      </a-col>
      <a-col :xl="20" :lg="18" :md="12" :sm="24" :xs="24" class="mb-3">
...
export default {
  ...
  methods: {
    ...
    onSelectCategory(categoryIds) {
      this.queryParam.categoryId = categoryIds[0]
      this.handleQuery()
    }
  }
}
